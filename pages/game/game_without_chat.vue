<template>
	<view class="gamePage" :style="{height:styleSetting.windowHeight+'px'}">
		<view class="gameBackground" :style="{width:styleSetting.windowWidth+'px'}">
			<view class="gameTitle">{{currentRound+" "+currentTurn}}</view>
			<view>
				...
				<view class="gameDetail">
					<view class="gameDetailScript">HURRY UP...The disaster happen anytime, Please faster decide !!!</view>
		...
		<uni-popup :show='showMessage' type="center" mode="middle">
		...
		<view class="gameButton" :style="{height:styleSetting.windowHeight*0.45+'px'}">
	...
	export default{
		data () {
			return {
				...
				money: 1000,
				index: 0,
				playerList: ['Player','user0001','user0002','user0003'],
		...
		methods: {
			...
			checkSeq: function() {
				// if (_self.currentTurn.split('n')[1] > 1){
					_self.emptySeq = ['-', '-', '-', '-', '-', '-', '-', '-', '-']
					_self.emptySeq.splice(0,_self.currentTurn.split('n')[1]-1,_self.currentSeq.slice(0,_self.currentTurn.split('n')[1]))
			...
			getHistoryResult: function(){
				...
				historyConnect.onSnapshot(function(doc){
					...
					if (_self.currentRound == 'round02') {
						console.log(Object.values(doc.data()['play_history'][_self.playerList[1]]).slice(10,Number(_self.currentTurn.split('n')[1])+9));
...
<style lang="scss">
	@import '@/static/game_without_chat.scss';
</style>
